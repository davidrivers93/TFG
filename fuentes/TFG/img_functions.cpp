...
void search_rectangles(std::vector<Rect> & validBBs,vector<vector<Point> > & contours , vector<Vec4i> & hierarchy, Size & s,std::vector<int> depth){
...
void get_bibs(std::vector<Rect> & validBBs, Size & s, std::vector<std::vector<Rect> > & dorsales, std::vector<std::vector<int> > & num){
	for (int i = 0; i < validBBs.size(); i++) {
		...
		for (int i2 = 0; i2 < validBBs.size(); i2++) {
			...
			if (ratio_mass_x > 1.2 || ratio_mass_x < 0.98)
				continue;

			if (ratio_width > 1.2 || ratio_width < 0.98)
				continue;
			...
			for (int i3 = 0; i3 < validBBs.size(); i3++) {
				...
				float ratio_width =float(bb_temp_3.width) / float(bb_temp_2.width);
...
void calculate(set<string> images, int contador,database_mng & database) {
	if (images.size()) {
		...
		for (it = images.begin(); it != images.end(); it++) {
			...
			search_rectangles(validBBs, contours, hierarchy, s, depth);
