...
void ayuda()
{
	...
	fprintf(stderr,"Uso: cimg_cierre  [opciones] imagen_in [imagen_out] \n");
	...
	fprintf(stderr,"Funcion: Realiza un cierre  con cuadrados o segmentos horizontales o verticales \n");
}
int main(int argc, char **argv)
{
	...
	CImg<unsigned char> cierre = entradaf.get_dilate(tamx,tamy).erode(tamx,tamy);
	...
	{
		cierre.save(argv[optind+1]);
		strcpy(titulo,argv[optind+1]);
	}

	if(pintarventana)
		cierre.display(titulo,0);
